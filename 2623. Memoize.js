...
function memoize(fn) {
    ...
    function getCacheKey(args) {
        if (fn.name === "sum") {
            // Ensure distinct key for different argument orders (a, b) and (b, a)
            return `${args[0]},${args[1]}`;
        }
        return args[0]; // For single-argument functions like fib or factorial
    }
...
const results = [];
const memoizedFn = fnName === "sum" ? memoizedSum : fnName === "fib" ? memoizedFib : memoizedFactorial;
